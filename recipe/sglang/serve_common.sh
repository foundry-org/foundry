...
topology_args() {  # $1 cfg -> sets N, ARGS, GRAPHS
  ...
  case "$cfg" in
    ...
    ep2|ep4|ep8)   N=${cfg#ep}
      ARGS="--tp-size $N --dp-size $N --ep-size $N --enable-dp-attention $EP_A2A --disable-custom-all-reduce
    ...
    tp2ep4|tp2ep8|tp4ep8) local atp=${cfg%%ep*}; atp=${atp#tp}; N=${cfg##*ep}
      ARGS="--tp-size $N --dp-size $(( N / atp )) --ep-size $N --enable-dp-attention $EP_A2A --disable-custom-all-reduce
            --chunked-prefill-size $EP_CHUNK $EP_ATTN --max-running-requests $(( 256 * N ))"
...
serve_main() {
  local cfg=${1:?Usage: $0 <cfg> [--save|--load]} mode=${2:-}
  ...
  case "$mode" in
    ...
    *)      echo "Usage: $0 <cfg> [--save|--load]"; exit 1 ;;
  esac
